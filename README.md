...
```
$ qemu-system-x86_64 -drive file=<fs_name>.img,index=0,media=disk,format=raw -m <memory_amount> -fsdev local,id=host_ds,path=<host/path/to/shared/folder>,security_model=none -device virtio-9p-pci,fsdev=host_fs,mount_tag=host_shared
```
