...
def build(project_path,
    ...
    mounted_wsgi_path = wsgi_path.replace(project_path, "").strip("/")
    ...
    create_gunicorn_start(mounted_project_path=mounted_project_path,
                          mounted_wsgi_path=mounted_wsgi_path.replace("/", "."),
                          app_name=app_name,
...
def create_gunicorn_start(mounted_project_path, mounted_wsgi_path, app_name, num_workers, template_path, create_path):
    ...
    create_f.write(template_f.read().replace("<APP_NAME>", app_name).replace("<NUM_WORKERS>", str(num_workers)).replace("<MOUNTED_WSGI_PATH>", mounted_wsgi_path).replace("<MOUNTED_PROJECT_PATH>", mounted_project_path))
    create_f.close()
    template_f.close()
